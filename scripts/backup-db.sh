...
# 🔴 **이 폴더는 VM 안이다** — VM 과 함께 사라진다.
# ✅ **호스트로 가져가지 않기로 했다**(2026-09-18 사용자 결정: *"최신 백업 가져올 필욘 없어 어짜피 이 vm에서 할 거니까"*).
#    ⚠ **그러니 권하지 않는다** — 매번 찍던 `scp` 안내를 2026-09-21 에 뺐다.
#    🔴 **대가는 적어 둔다**: VM 자체를 잃으면 **백업도 함께 잃는다**(infra/README 「VM 안의 백업은 백업이 아니다」의 그 경우).
#    되돌리려면 호스트(PowerShell)에서: `scp -P 2222 "ecstel@127.0.0.1:<백업폴더>/*-<STAMP>.*" .`
#
...
# ⚠ 여기서 «호스트로 가져가라» 를 **찍지 않는다** — 2026-09-18 에 안 하기로 정했고,
#    매번 권하면 결정이 안 선 것처럼 보인다(머리말의 «대가» 참조).
echo "✓ 백업 성립 — $STAMP (최근 ${KEEP}벌 보관 · VM 안)"
exit 0
