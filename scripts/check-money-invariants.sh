...
-- ⑫⑬ 탈퇴한 회원을 가리키는 행이 없다 — DB 에 FK/CASCADE 가 없어 **앱이 지우는 수밖에 없다**.
--    🔴 **원인이 둘이다 — 이름만 보고 리스너를 의심하지 말 것**(2026-09-21 실측):
--      ① 탈퇴 리스너가 **빠뜨린 것**(원래 이 불변식이 겨냥한 자리)
--      ② 🔴 **탈퇴 «뒤» 에 새로 생긴 것** — `purge` 가 지나간 뒤 관리자 조작이 새 행을 만든다.
--         2026-09-21 에 «탈퇴 회원의 주문을 관리자가 취소» 를 **처음으로 실제로 밟자** ⑬ 이 1 이 됐고,
--         그 알림은 **아무도 영원히 못 읽는다**(보관 정리가 180일 뒤 지운다).
--    ⚠ 둘은 **고치는 자리가 다르다** — ①은 리스너, ②는 «생성 시점에 회원이 있나» 다.
--      WA §1-3 의 *"결론이 맞아도 이유는 틀릴 수 있다"* 가 이 줄에 붙어 있다.
