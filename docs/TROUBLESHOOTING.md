...
| 🔴 **주석을 분명히 썼는데 IDE 호버·생성 문서에 안 뜬다** — 컴파일은 통과하고 경고도 없다 | **바로 앞에 javadoc 블록이 하나 더 있다.** 둘이 연달아 있으면 자바는 **뒤엣것만** 선언에 붙이고 앞 블록은 조용히 버린다 — 사이에 빈 줄이 있어도 마찬가지다. 생기는 경위는 둘이다: ①새 메서드·상수를 **기존 주석과 선언 사이에** 끼워 넣었다(`Order#cancelItem` — 09-02 K-3 의 헬퍼가 사이에 들어갔다) ②딸린 선언이 **다른 파일로 옮겨 가고** 주석만 남았다(`OrderService` 에 남은 `DETAIL_MAX` 유품) | `scripts/check-orphan-javadoc.sh` (0=없음 / 1=발견, 인자로 다른 트리도 검사한다). 🔴 고치는 법은 「지운다」가 아니라 대개 **붙을 선언 바로 위로 옮긴다** — `@return` 이 달려 있으면 구역 머리말일 수가 없으니 그것부터 본다. ⚠ **`/* */` 로 쓴 구역 머리말은 정상이라 세지 않는다**: 블록 종류를 구분하지 않고 닫는 `*/` 만 세면 이미 올바른 머리말이 전부 오검출된다 — 첫 판이 그렇게 틀려 «10곳» 이라 했다가 실제로는 2곳이었다. 2026-09-03 실측 7건(주문 도메인에 몰려 있었다) · `6fc3dde` |
