...
| ⚠ **숫자 입력칸을 비웠는데 `0` 이 된다**(DevExtreme `DxNumberBox`) — 비울 방법이 없어 저장이 막히거나, **비우려던 값이 조용히 0 으로 저장된다** | 🔴 **`format` 이다.** 서식(`format="#,##0"`)이 걸리면 마스크 입력기가 되고 **빈 입력이 `0` 으로 확정된다**(2026-09-21 격리 실측). ⚠ **`min` 이 아니다** — 2026-08-13 에 `:min="0"` 을 떼 봤지만 그대로여서 «원인 미상» 으로 5주 넘게 남았다. `min` 만 걸면 안 그렇고, **`format` 만 걸어도 그렇다** | **가르는 법**: `DxNumberBox` 를 맨몸으로 띄워 조합을 갈라 본다(`format` 만 / `min` 만 / 둘 다 / 없음) — `views/ProductFormView.test.js` 의 「범인은 `format` 이다」가 그 형태다. → **고치려면 `format` 을 빼야 하고** 천단위 구분을 잃는다. 🔴 **0 이 «없음» 으로 읽히는 칸이면 «뜻을 넓혀» 우회하는 편이 싸다**(정가 칸이 그 선택) · ⚠ **0 이 유효한 값인 칸(재고 등)은 우회가 안 된다** — BACKLOG §H-9 · `handoffs/2026-09-21-handoff.md` §9 |
