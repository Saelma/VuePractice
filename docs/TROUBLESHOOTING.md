...
| 증상 | 의심 | 어디 |
|---|---|---|
| `BUILD SUCCESSFUL` 인데 실제로 아무것도 안 돌았다 | `.env` 미소싱(통합 테스트가 `@EnabledIfEnvironmentVariable(named="DB_HOST")` 라 **조용히 SKIPPED**) · `Task :test UP-TO-DATE`(환경변수는 Gradle 입력이 아니다) | WA §3 — 판정은 `tests`·`skipped` **숫자로만**. `set -a; . /home/ecstel/work/.env; set +a` + `cleanTest` + `python3 -c "import glob,xml.etree.ElementTree as ET; print(sum(int(ET.parse(p).getroot().get('skipped')) for p in glob.glob('build/test-results/test/*.xml')))"` · 🔴 **곁증거 하나**(2026-08-14): 새 마이그레이션을 넣었는데 `flyway_schema_history` 최신 버전이 그대로면 **컨텍스트가 안 떴다는 뜻**이다 — V51 이 그렇게 드러났다 · 🔴 **네 번째다 — 2026-08-28 엔 그대로 커밋·배포까지 갔다**(07-21 · 08-04 · 08-14 에 이어. 앞의 셋은 같은 세션 안에서 잡혔다). 커밋 메시지의 «백엔드 1013» 은 **선언된 수**였고 그중 **488 이 SKIPPED** 였다 — «1013» 이라는 값이 실측처럼 보여서 아무도 다시 안 셌다(WA §4-3-1 의 형태). ⚠ **판정은 `tests` 가 아니라 `skipped` 로** 한다 · `handoffs/2026-08-14-handoff.md` §11-4 · `handoffs/2026-08-28-handoff.md` §4 |
