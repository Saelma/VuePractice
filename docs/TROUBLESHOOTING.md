...
| 증상 | 의심 | 어디 |
|---|---|---|
| 🔴 **어떤 SQL 이 나가는지 보고 싶은데 로그가 안 켜진다** | **물어볼 곳을 틀리게 잡았다.** 실행 로그에서 «건져 올리려» 하면 설정 싸움이 된다(테스트엔 SQL 로깅이 없고 `--info` 로도 `LOGGING_LEVEL_ORG_HIBERNATE_SQL` 로도 안 켜진다 — 2026-09-02 실측) | **같은 사실을 «더 이른 곳» 에서 묻는다** — 설정·방언·계약. 잠금 절이라면 `Dialect#getReadLockString`·`getWriteLockString` 가 답을 갖고 있다(Oracle 은 둘 다 ` for update` — 그래서 `PESSIMISTIC_READ` 로 낮춰도 동작이 같다). 🔴 **경합을 만들 필요도, 로깅을 켤 필요도 없었다.** ⚠ 확인한 것은 탐침 테스트로 남긴다(`LockModeSqlProbeTest`) — 전제가 바뀌면 빨개져 알려 준다 · WA §3-3-4 · `handoffs/2026-09-02-handoff.md` §9-4 |
| `BUILD SUCCESSFUL` 인데 실제로 아무것도 안 돌았다 | `.env` 미소싱(통합 테스트가 `@EnabledIfEnvironmentVariable(named="DB_HOST")` 라 **조용히 SKIPPED**) · `Task :test UP-TO-DATE`(환경변수는 Gradle 입력이 아니다) | WA §3 — 판정은 `tests`·`skipped` **숫자로만**. `set -a; . /home/ecstel/work/.env; set +a` + `cleanTest` + `python3 -c "import glob,xml.etree.ElementTree as ET; print(sum(int(ET.parse(p).getroot().get('skipped')) for p in glob.glob('build/test-results/test/*.xml')))"` · 🔴 **곁증거 하나**(2026-08-14): 새 마이그레이션을 넣었는데 `flyway_schema_history` 최신 버전이 그대로면 **컨텍스트가 안 떴다는 뜻**이다 — V51 이 그렇게 드러났다 · 🔴 **다섯 번째다**(07-21 · 08-04 · 08-14 · 08-28 · **2026-09-02**). 08-28 엔 그대로 커밋·배포까지 갔다. ⚠ **09-02 는 모양이 새로웠다** — 전수가 아니라 **변형 주입**에서 물렸다: 변형 셋을 넣었는데 «빨간 0건 · BUILD SUCCESSFUL» 이라 «이 변형은 아무도 안 잡는다» 로 판정할 뻔했다. 🔴 **«안 잡힘» 과 «안 돌았음» 이 같은 숫자로 보인다** — 그래서 판정에 «몇 건이 돌았나» 를 함께 넣는 규약이 생겼다(WA §변형주입). 커밋 메시지의 «백엔드 1013» 은 **선언된 수**였고 그중 **488 이 SKIPPED** 였다 — «1013» 이라는 값이 실측처럼 보여서 아무도 다시 안 셌다(WA §4-3-1 의 형태). ⚠ **판정은 `tests` 가 아니라 `skipped` 로** 한다 · `handoffs/2026-08-14-handoff.md` §11-4 · `handoffs/2026-08-28-handoff.md` §4 |
...
| DevExtreme 위젯을 테스트에서 **눌렀는데 아무 반응이 없다**(`opened` 가 늘 `false`) — 또는 🔴 **깜빡임을 고쳤는지 테스트가 못 가른다** | 🔴 **`w.find(...).trigger('click')` 은 DevExtreme 에 안 닿는다** — 위젯은 네이티브 `click` 이 아니라 자기 이벤트(`dxclick`, 포인터 기반)로 열고 닫는다. 그래서 «안 눌린 것» 이 **«안 열린다» 로 보인다**(2026-09-11: 세 설정이 전부 `[false,false,false]` — 판정 불가였다). ⚠ 둘째: **닫혔다가 곧바로 다시 열리면 `closed` 이벤트가 안 온다** — 닫힘이 끝나기 전에 열려서다. 최종 상태도 «열림» 이라 **깜빡임이 테스트에서 안 보인다** | 클릭은 `import eventsEngine from 'devextreme/events/core/events_engine'` → `eventsEngine.trigger(inputEl, 'dxclick')` · 화면이 네이티브 클릭도 받으면 **둘 다** 보낸다(`inputEl.dispatchEvent(new MouseEvent('click', { bubbles: true }))`) · 깜빡임은 **`inst.on('optionChanged', e => e.name === 'opened' && !e.value)` 로 «닫힘으로 바뀐 횟수»** 를 센다(`closed` 로 세면 변형 주입이 통과했다) · `views/AuditLogAdminView.test.js` · `handoffs/2026-09-11-handoff.md` §9 |
| DevExtreme **팝업 목록(드롭다운)의 모양**을 봐야 하는데 jsdom 에선 목록이 안 그려진다 · 사용자 화면에서만 **항목 몇 줄이 한 칸 밀려** 보인다 | ①jsdom 은 팝업 내용을 그리지 않는다(`.dx-list-item` 0 — 위 줄). ②🔴 **밀림은 `float` 를 의심한다** — DevExtreme 은 «전체 선택» 체크박스를 `float: left` 로 띄운다(`.dx-list-select-all-checkbox`). 떠 있는 요소가 자기 줄을 벗어나면 **옆을 지나는 줄들이 그 너비만큼 밀린다**(2026-09-11: 회원·주문 분류만 밀리고 «전체 선택» 체크박스가 회원 곁에 떠 있었다). ⚠ **브라우저마다 다르다** — 같은 코드를 Firefox 로 찍으면 멀쩡했다 | 화면을 직접 보려면 **임시 빌드 + Firefox 헤드리스 스크린샷**: 컴포넌트 하나를 띄우는 진입점을 따로 두고 `npx rsbuild build -c <임시 설정>`(출력은 `assetPrefix: './'`) → `firefox --headless --window-size=700,900 --screenshot out.png file://…/index.html`. ⚠ **`load` 순간에 찍힌다** — 열기·측정은 동기로 하고(`fx.off = true` 로 애니메이션을 끈다) 재 본 값은 페이지에 글자로 찍어 스크린샷에 담는다. ⚠ 🔴 **재현이 안 되면 사용자에게 스크린샷을 받는다** — 이번엔 그게 원인을 갈랐다 · `handoffs/2026-09-11-handoff.md` §10 |
| 전수가 **어제까지 초록이던 자리에서** 빨개졌다(내 변경과 무관해 보인다) | 공유 espdb 라 **어제 남긴 검증 잔재가 오늘 테스트에 보인다.** 🔴 **여기서 갈래가 둘이고 대응이 반대다**: ①**테스트의 정의가 틀렸다** — 운영 데이터는 그걸 «드러냈을 뿐» 이다 → **고친다** ②**테스트의 전제가 운영에 깨졌다**(«그 상태가 없다» 를 가정했다) → **견디게 만든다**. ⚠ 둘을 같은 손짓으로 넘기면 ①이 영영 안 고쳐진다 — *"또 운영 데이터네"* 로 지나간다 | 가르는 법: **그 테스트가 무엇을 묻는 이름인지** 본다. *"코드와 SQL 이 같은 정의인가"* 류면 ①을 먼저 의심하고 **서비스 쪽 조건과 대조 쿼리를 한 줄씩 맞춰 본다**. 2026-08-14 실측: 저재고 대조 SQL 에 `deleted_at IS NULL` 이 없어(F-7 을 08-12 에 놓쳤다) **정의가 다른 채로 초록**이었고, 「삭제 대기이면서 저재고」인 행이 운영에 처음 생긴 날 드러났다. 같은 날 ②도 함께 났다(어제 검증 쿠폰의 발급 창이 겹쳐 **생성 자체가 거부**) · 🔴 **2026-09-01 에 ①이 가장 선명한 모양으로 또 났다**: 브라우저로 「가입 쿠폰」을 처음 지정하자 그 다음 전수에서 `WelcomeCouponIntegrationTest` **셋이 `ORA-00001`**(`ux_coupon_welcome`)로 죽었다. 원인은 잔재가 아니라 **운영 코드의 flush 순서**였다 — `setWelcome` 이 «기존 해제 → 새 지정» 을 같은 트랜잭션에 맡기는데 **Hibernate 는 필드를 고친 순서가 아니라 자기 순서로 UPDATE 를 낸다.** ⚠ **그 분기는 «이미 지정된 쿠폰이 있을 때» 만 도는데 지정이 32일간 0 이라 한 번도 안 밟혔고**, 테스트가 초록이던 것은 **옛 쿠폰이 영속성 컨텍스트에 먼저 올라와 있어서 생긴 우연**이었다. → 해제 직후 `flush()` + `entityManager.clear()` 로 순서를 결정적으로 만드는 재현 테스트 · `handoffs/2026-08-14-handoff.md` §6 · `handoffs/2026-09-01-handoff.md` §11 |
| 🔴 **주석을 분명히 썼는데 IDE 호버·생성 문서에 안 뜬다** — 컴파일은 통과하고 경고도 없다 | **바로 앞에 javadoc 블록이 하나 더 있다.** 둘이 연달아 있으면 자바는 **뒤엣것만** 선언에 붙이고 앞 블록은 조용히 버린다 — 사이에 빈 줄이 있어도 마찬가지다. 생기는 경위는 둘이다: ①새 메서드·상수를 **기존 주석과 선언 사이에** 끼워 넣었다(`Order#cancelItem` — 09-02 K-3 의 헬퍼가 사이에 들어갔다) ②딸린 선언이 **다른 파일로 옮겨 가고** 주석만 남았다(`OrderService` 에 남은 `DETAIL_MAX` 유품) | `scripts/check-orphan-javadoc.sh` — 인자 없으면 워킹트리, **커밋을 그대로 줘도 된다**(`… 6fc3dde^` — 그 시점 `src` 만 임시로 꺼내 본다). 0=없음 / 1=발견 / 2=인자 오류. 🔴 고치는 법은 「지운다」가 아니라 대개 **붙을 선언 바로 위로 옮긴다** — `@return` 이 달려 있으면 구역 머리말일 수가 없으니 그것부터 본다. ⚠ **`/* */` 로 쓴 구역 머리말은 정상이라 세지 않는다**: 블록 종류를 구분하지 않고 닫는 `*/` 만 세면 이미 올바른 머리말이 전부 오검출된다 — 첫 판이 그렇게 틀려 «10곳» 이라 했다가 실제로는 2곳이었다. 2026-09-03 실측 7건(주문 도메인에 몰려 있었다) · `6fc3dde` |
| 🔴 **`sed`·`grep -E` 가 «바꿨어야 하는데 원문 그대로»다** — 오류도 없고 종료코드도 0 이다. ⚠ 짧은 문자열로 따로 시험하면 **잘 된다** | 🔴 **로케일이다.** `ko_KR.UTF-8` 에서 GNU sed 4.8 의 `s/…​.*$//` 가 **한글·이모지가 섞인 긴 문자열**에서 매치에 실패하고 **조용히 넘어간다**(2026-09-21 실측: 같은 입력이 로케일에선 **828바이트**, `LC_ALL=C` 에선 **18바이트**). ⚠ **증상이 «내 정규식이 틀렸나» 로 보여서** 패턴을 고치며 시간을 쓴다 — 실제로 그렇게 «한 줄에 항목이 여럿이라 그렇다» 로 **한 번 오진했다**(WA §1-3: 결론이 맞아도 이유는 틀릴 수 있다) | **가르는 법 한 줄**: `printf '%s' "$s" | LC_ALL=C sed -E '…' | wc -c` 와 로케일 그대로의 결과를 **바이트 수로** 견준다 — 다르면 로케일이다. → **문서·로그 본문을 훑는 자리는 `LC_ALL=C` 로 고정**한다(바이트 기준이라 한글도 `grep -F` 로 잘 찾는다). `scripts/check-stale-residue.sh` 의 `SCAN()` 이 본보기 · `handoffs/2026-09-21-handoff.md` §3 |
...
| 🔴 배포된 **파일을 직접** grep 했더니 새 문구가 **0건** | **못 읽은 것이다.** `deploy-frontend.sh` 가 웹루트를 `root:nginx` 로 바꾸므로(`chown -R root:nginx`) 일반 사용자의 `grep -r /var/www/...` 는 **「허가 거부」를 stderr 로 흘리고 0건처럼 보인다** — «배포 안 됐다» 로 오진한다(2026-08-25 실측) | **파일이 아니라 nginx 가 서빙하는 것을 받는다**: `curl -sk https://127.0.0.1/` → 번들 경로를 뽑아 `curl` 로 받아서 센다. 🔴 **그게 «실제로 나가는 것» 이라 더 맞는 근거이기도 하다.** ⚠ `grep -r` 은 실패를 조용히 넘기므로 **0 을 세기 전에 `2>&1` 로 한 번 본다**(WA §3-3) · `handoffs/2026-08-25-handoff.md` §8-1 |
| 🔴 nginx 가 **서빙하는 번들**을 `curl` 로 받아 grep 했는데도 새 문구가 **0건** | **문자 이스케이프다.** 빌드가 비ASCII 문자를 JS 문자열 이스케이프로 내보낸다 — 가운뎃점 `·` 은 번들에 **`\xb7`** 로 실린다(2026-08-26 실측: 소스의 `취소·반품된 몫은` 이 번들에선 `" 취소\xb7반품된 몫은 "`). 한글은 그대로라 **한글만 있는 문구는 잡히고 특수문자가 낀 문구만 0건**이 되어 더 헷갈린다 | **특수문자를 뺀 조각으로 다시 센다** — `grep -c "반품된 몫은"`. 🔴 배포 여부는 **한글만으로 된 조각**(예: `상점의 인기순과 같은 기준`)으로 판정하는 게 안전하다. ⚠ 실린 모양을 직접 보려면 `grep -o '.\{40\}조각.\{80\}'` · `handoffs/2026-08-26-handoff.md` §6-1 |
| 🔴 배포된 **jar 안의 클래스**를 `strings` 로 뒤졌는데 한글 문구가 **0건** | **`strings` 가 건너뛴 것이다.** 기본 동작이 «인쇄 가능한 ASCII 가 4자 이상 이어진 곳» 이라 **한글(UTF-8 다바이트)은 애초에 후보가 아니다**(2026-08-26 실측: 같은 클래스에서 SQL 은 잡히고 「마지막 품목」은 0건). ⚠ 셋째 형제다 — 앞의 둘은 「허가 거부」와 「문자 이스케이프」 | **`grep -a` 로 센다**(바이너리를 텍스트로 취급). 확실히 보려면 `javap -v -p <class> | grep -a '조각'` — **상수풀에 그대로 뜬다**(`#560 = Utf8  (마지막 품목 — 주문 취소됨)`). 🔴 클래스에 문자열이 있는지는 이 방법이 결정적이다 · `handoffs/2026-08-26-handoff.md` §9-1 |
...
| **«브라우저로 확인했다» 가 사실인지 나중에 가려야 한다** | 저널은 날아가도 **nginx 접근 로그**(열었나)와 **`admin_audit_log`**(무엇을 했나)는 디스크에 남는다. ⚠ **전수 실행 시각과 대조해** 테스트가 만든 행과 가른다 | **§2-5 (아래)** |
...
### 2-5. 🔴 «사람이 브라우저에서 정말 했나» 를 나중에 가르는 법

**증상**: 기록에 *"브라우저로 확인했다"* 가 없다(또는 있는데 못 믿겠다). 저널은 날아갔다.

🔴 **«저널이 휘발성» 을 «로그가 없다» 로 읽지 말 것.** 디스크에 남아 **재부팅에도 사는** 곳이 둘 있다.
...
→ **전수를 돌린 시각과 대조**한다. 그보다 뒤면 사람이 한 것이다.

> **어디서 왔나**: 2026-09-07 의 브라우저 검증 여부를 09-10 에 **nginx 로그로** 갈랐고
> (`handoffs/2026-09-10-handoff.md` §2-1), 같은 날 R 축 배포 검증을 **감사 로그로** 확인했다(§16-10).
> ⚠ **두 번째 관측에서야 옮겼다** — 한 건일 때는 안 옮겼다.
