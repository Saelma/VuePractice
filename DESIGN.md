...
**훔칠 것 — 지금 우리에게 없는 것부터** (✅ 2026-07-29 적용분 표시)
...
2. ⏳ **상품 카드의 한 줄 태그라인** (미적용 — 마이그레이션이 붙어 뒤로 미룸). 한글명 + 영문명 + *「안 입은 듯한 무압박 브라 & 팬티」* 같은 짧은 설명이
   카드에 함께 온다. 우리 `ProductCard` 는 이름·가격·별점뿐이라 **무엇이 좋은 상품인지 목록에서 안 읽힌다.**
   ⚠ 넣으려면 `product` 에 한 줄 카피 필드가 필요하다 — 상세 설명을 잘라 쓰면 문장이 잘려 지저분해진다.
