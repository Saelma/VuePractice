...
1. ✅ **신규가입 혜택을 첫 화면에 박는다** (적용 2026-07-29 — 핸드오프 §7-2). 8fter 는 헤더/히어로에 *「신규가입 혜택 | 쿠폰팩 3만원 + 무료배송」*
   을 상시 노출한다. ⚠ **우리는 쿠폰·적립금·무료배송 기준이 다 있는데 비로그인 첫 화면에서 그 사실을
   전혀 알리지 않는다** — 혜택 허브(`/benefits`)는 로그인 후에만 보인다. **기능이 아니라 노출의 문제**다.
2. ✅ **상품 카드의 한 줄 태그라인** (적용 2026-07-29, V33 — 핸드오프 §8). 한글명 + 영문명 + *「안 입은 듯한 무압박 브라 & 팬티」* 같은 짧은 설명이
   카드에 함께 온다. 우리 `ProductCard` 는 이름·가격·별점뿐이라 **무엇이 좋은 상품인지 목록에서 안 읽힌다.**
   ⚠ `description` 을 잘라 쓰지 않고 **별도 필드**(`product.tagline`, 100자)로 만들었다 — 설명은 문단이라
   앞 N자를 자르면 문장이 끊긴다. **카드용 한 줄과 상세 설명은 목적이 다른 글**이다.
   화면 규칙: 없으면(대부분) **줄 자체를 없앤다**(`v-if`) — 빈 줄을 남기면 카드 높이가 갈려 그리드가 흔들린다.
...
- **컬렉션/드롭 중심 홈 서사**(「26SS 3rd Collection」 같은 시즌 내러티브)는 **패션 브랜드의 문법**이다.
...
> ⚠ **쿠폰은 광고하지 않았다** — 8fter 는 「가입 쿠폰팩」을 내걸지만 우리 쿠폰은 관리자 지정 발급만이라
