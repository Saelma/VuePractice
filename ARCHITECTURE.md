...
> **소형 3건 (2026-08-03, B-20·B-22·B-23)**:
>
> **B-20 배송 요청사항 (V38)** — `orders.ship_memo VARCHAR2(200 CHAR)`. **주문 스냅샷**이라
> `ship_*` 5컬럼 옆에 둔다: 주소록을 고쳐도 **과거 주문의 요청은 그대로**여야 한다(V18 이 배송지를
> 주소록으로 옮기면서도 `orders.ship_*` 는 안 건드린 것과 같은 판단).
> ⚠ 화면에서 요청사항을 **배송지 폼 밖**에 뒀다 — 「기본 배송지로 저장」이 부르는
> `updateShippingAddress` 에 섞이면 **주문 단위 값이 주소록에 저장된다.** 규율이 아니라 구조로 막았다.
> ⚠ DTO `@Size(max=200)` 이 DB 컬럼과 같아야 한다 — 더 헐거우면 **ORA-12899 로 주문이 통째로 실패**한다.
>
> **B-22 리뷰 정렬·사진만 (마이그레이션 없음)** — ⚠ **정렬은 원래부터 있었다**
> (`ReviewRepositoryImpl.SORTABLE`). 화면이 `?sort` 를 안 보냈을 뿐이다. 새로 만든 건 `photoOnly` 뿐.
> 조건을 **`BooleanBuilder` 한 곳**에서 만들어 목록·카운트가 갈리지 않게 했다(B-16 에서 변형이
> 잡았던 자리를 이번엔 설계로 막았다).
> ⚠ **요약 통계는 필터를 타지 않는다** — 필터마다 평균 별점이 달라지면 **상품 카드의 별점과 어긋나
> 같은 상품인데 화면마다 다른 평점**이 뜬다. 목록에만 필터를 건다.
>
> **B-23 연관 상품 (백엔드 변경 0)** — 기존 목록 API 를 `categoryId` + `soldCount,desc` 로 부른다.
> **추천 로직이 아니라 "같은 분류에서 잘 팔리는 것"** 이라 서버가 새로 해 줄 일이 없다.
> ⚠ **"함께 구매한 상품" 은 하지 않는다** — 주문이 수십 건이라 추천이 성립하지 않는다(§1).
>
> ⚠ **공통 교훈**: `PageableExecutionUtils`(`QueryDslSupport.page`)는 **첫 페이지 결과가 페이지
> 크기보다 작으면 count 쿼리를 아예 실행하지 않는다.** 그래서 `totalElements` 를 검증하는 테스트는
> **결과 수 > 페이지 크기** 를 만들어야 count 경로를 탄다 — 안 그러면 count 조건을 지워도
> 테스트가 통과한다(2026-08-03 변형 주입에서 실제로 걸렸다, WA §3).

> **재고 변경 이력 (2026-08-04, B-19, V39)**: `product_variant.stock` 은 **현재값 하나**뿐이고
> 주문·취소·반품·관리자 편집이 전부 같은 컬럼을 덮어써서 *"어제 10개였는데 왜 3개지"* 에 답할 수
> 없었다. `point_history` 가 적립금 잔액의 **원장**인 것과 같은 구조를 재고에 둔다(`stock_history`,
> append-only, **`SUM(quantity)` = 현재 재고**).
>
> 🔴 **`variant_id` 는 안정적인 키가 아니다 — 이 설계의 전부가 여기서 갈렸다.**
> `ProductCommandService.update()` 는 옵션을 **통째로 교체**하고(`deleteAll` + `saveVariants`)
> `ProductVariant.of()` 가 새 엔티티를 만들어 PK 를 새로 발급한다. **관리자가 상품을 한 번만 저장해도
> 모든 옵션의 id 가 바뀐다.** 옵션 id 로 이었다면 편집 한 번에 이력이 끊기는데, **화면은 빈 목록으로
> 멀쩡히 뜬다**(에러가 아니라 조용한 소실).
> → `order_item` 이 같은 문제를 이미 스냅샷으로 풀었다(`variant_name`). 같은 방식으로
> **조회 기준은 `product_id` + `variant_name` 스냅샷**이고 `variant_id` 는 **느슨한 참조**로만 남긴다
> (FK 아님·NULL 허용 — 이력이 옵션보다 오래 살아야 한다). 편집의 전/후 대조도 **옵션명**으로 한다.
> → 버린 안: 옵션 교체를 부분 갱신으로 바꿔 `variant_id` 를 안정시키기. 주문·장바구니가 얹혀 있어
> 범위를 넘고, 통째 교체를 고른 이유가 이미 코드에 적혀 있다 — **B-19 때문에 그 결정을 뒤집지 않는다.**
>
> **기록 지점은 다섯인데 전부 `ProductCommandService` 를 지난다** — 주문·취소·반품은
> `decreaseStock`/`increaseStock`, 관리자 등록·편집은 `create`/`update`. 한 클래스에 모이므로
> 빠지는 경로가 없다. 사유는 `ORDER`(−) · `CANCEL`(+) · `RETURN`(+) · `ADMIN_CREATE`(+) · `ADMIN_EDIT`(±).
> **부호는 `reason` 으로 유추하지 않고 `quantity` 에 담는다**(`point_history.amount` 와 같은 규칙 —
> 그래야 합계를 `SUM` 으로 낸다).
> ⚠ **행위자는 관리자 조작에만 남긴다** — 주문 경로는 `order_id` 로 되짚을 수 있어 같은 정보를
> 두 번 적지 않는다. 그래서 **주문 차감을 `orders.save()` 뒤로 옮겼다**(적립금 차감이 이미 그 자리에
> 있던 것과 같은 이유 — 주문보다 먼저 차감하면 근거 없는 행이 된다).
> ⚠ **변동 0 은 기록하지 않는다.** 상품명만 고쳐도 옵션은 다시 만들어지므로, 안 걸러내면 저장할 때마다
> 줄이 쌓여 진짜 변동이 묻힌다.
> ⚠ **백필하지 않았다** — 과거 변동은 *기록이 없는 게 사실*이고, 현재 재고를 첫 줄로 지어 넣으면
...
> (`requestReturn(id, memberId, reason)` vs `cancel(id, memberId)`) 관리자가 **왜 취소됐는지**
> 알 수 없었다. `orders.cancel_reason` 을 `return_reason`(V24)과 **같은 길이·semantics·nullable** 로 둔다 —
> 같은 성격의 값이 갈리면 한쪽에서 되던 입력이 다른 쪽에서 ORA-12899 로 터진다(WA §2-2-1).
>
> ⚠ **사유는 선택이다**(반품은 필수). 취소는 돈이 오가기 전 단계라 입력을 강제하면 마찰이 값보다 크다.
> 그 결정이 **세 자리로 번진다**: 컨트롤러 `@RequestBody(required = false)`(빼면 본문 없이 보내던
> **기존 취소가 전부 400**) · 엔티티에서 공백 → `null`(안 하면 화면이 *"사유가 있다"* 로 읽어 빈 칸을
> 그린다) · 프론트가 빈 문자열을 `null` 로 변환.
> ⚠ **행위자 컬럼은 두지 않았다** — 착수 실측에서 **관리자 취소 API 가 없다**는 게 나왔고
> (`/api/admin/orders` 는 GET 3개뿐, `cancel` 은 `findByIdAndMemberId`), 그래서 취소자는 항상
> 주문자 본인이다. 관리자 취소를 만들 때 함께 추가한다(BACKLOG B-25).
> ⚠ **재고 이력에 사유를 복사하지 않는다** — 원장의 `CANCEL` 줄은 `order_id` 를 갖고 있어 되짚어진다.
> 같은 정보를 두 곳에 적으면 갈릴 수 있다(V39 가 주문 경로에 행위자를 안 남긴 것과 같은 규칙).

> **관리자 리뷰 숨김 (2026-08-04, B-18, V41)**: 리뷰 컨트롤러 전수가 작성자 본인용 넷뿐이라
> **관리자가 부적절한 리뷰에 손댈 방법이 없었다**. `review.hidden` 을 두고 **숨김**으로 처리한다 —
> **삭제가 아닌 이유**는 관리자가 잘못 판단할 수 있고 그때 원문이 남아 있어야 되돌리기 때문이다.
>
> 🔴 **이 설계의 전부는 «같은 컬럼을 세 곳이 다르게 다룬다»** 이다:
> ①**상품 리뷰 목록** → 뺀다(⚠ 작성자 본인에게도 — 조회 조건이 한 줄로 끝나 갈릴 자리가 없다)
> ②**평균 별점·리뷰 수** → 뺀다. 안 빼면 **보이지도 않는 리뷰가 별점을 끌어내리는데 화면은
> 멀쩡하다** — 그래서 숨김·해제도 `ReviewRatingChangedEvent` 를 발행해 집계를 다시 맞춘다.
> ③🔴 **상품당 1회 제한** → **그대로 센다.** 셋 중 유일하게 안 빼는 자리다 — 빼면 숨기자마자
> 작성자가 새 리뷰를 써서 숨김이 무의미해진다.
>
> **관리자 목록은 리포지토리 메서드부터 나눈다**(`findByProduct` ↔ `findForAdmin`). 고객 목록은
> *"숨긴 것을 빼는"* 게 규칙이고 관리자는 *"숨긴 것을 봐야 되돌리는"* 게 규칙이라 **조건이 반대**다 —
> 한 메서드에 「누구에게 보이나」 플래그를 더하면 그 규칙이 호출부마다 갈린다.
> 상품명은 catalog 의 `ProductQueryService.findByIds` 로 **한 번에 모아** 가져온다(엔티티 조인은
> 도메인 경계를 깨고, 리뷰마다 조회하면 N+1 이다).
>
> ⚠ **V41 은 백필한다**(`DEFAULT 0`) — V37·V39·V40 과 반대다. 기존 리뷰는 **실제로 숨겨진 적이
> 없으므로 0 이 사실**이기 때문이다(V34 `email_verified` 와 같다). *"모르는 값"* 이 아니라
> *"아니다"* 라서 NOT NULL 로 둘 수 있고, 그래야 조회 조건에 NULL 갈래가 안 생긴다.
> ⚠ **감사 로그는 붙이지 않았다** — `AdminActionEvent` 가 회원 대상 설계(`targetId`=대상 회원)라
> 리뷰에 맞지 않고, 맞추려면 `AuditAction` 확장 + CHECK 마이그레이션이 따라온다.
> ⚠ **신고 기능은 일부러 뺐다** — 사용자가 있어야 의미가 생긴다(아무도 안 누르는 버튼과 빈 검토 큐).
