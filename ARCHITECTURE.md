...
>
> 🔴 **①의 「명시적 열거」가 2026-08-10 에 다시 어긋난 것이 확인됐다 — 아직 안 고쳤다.**
> `DELIVERED`(V13, **07-23**)가 열거에 안 들어와 **배송완료 고객이 리뷰를 못 쓴다**(운영 8조합).
> 판단 자체(자동 포함 금지)는 유지하되, **opt-in 은 주석으로 지켜지지 않는다** — 그걸 지킬 장치가
> 함께 있어야 한다. 열거를 지키던 통합 테스트도 **같은 모양이라 같이 낡았다**.
> 경위·고칠 때 정할 것: `handoffs/2026-08-10-handoff.md` §16-2.
>
