...
| **비밀번호 재설정** (B-10) | ✅ **완료** (2026-07-28) | — | "비밀번호 잊음" 흐름. 아이디로 **단발성 토큰** 발급(`PasswordResetTokenStore`, Redis `auth:reset:<token>`=memberId, 30분 TTL, `getAndDelete`로 재사용·경합 차단) → 새 비번. `RefreshTokenStore`와 반대로 **토큰을 키**로 둬(회원당 1개가 아니라 토큰당 1개) 재발급이 이전 링크를 조용히 무효화하지 않고 O(1) 소비. 변경 시 refresh 폐기(다른 세션 무효화, `changePassword`와 동일). **열거 방지**: 없는 아이디도 200. ⚠ **발송 채널(메일/SMS)이 없어** 여전히 반쪽 — 단 **보낼 주소는 생겼다**(B-13, 2026-07-29: 신규 가입 필수 + `/settings` 등록. 저장값은 소문자 정규화, 확인 메일이 없어 **미검증** 상태). 링크를 **dev 프로파일에서만** 응답에 실어(`auth.password-reset.expose-token`, base=false·`application-dev.yml`=true) 화면에서 확인한다. 운영은 기본 프로파일로 떠(systemd `ExecStart`에 `--spring.profiles.active` 없음) 절대 노출 안 됨. 마이그레이션 없음(Redis). 상세 `handoffs/2026-07-28-handoff.md` |
