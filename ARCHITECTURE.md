...
- **왜 UUID**: 서비스가 중앙 시퀀스 없이 각자 ID 생성 → MSA 친화적. ID로 업무정보(**순번**) 노출 안 됨 — ⚠ **다만 «아무것도» 노출 안 되는 건 아니다. 아래 「대가」 참조.**
...
- 🔴 **대가 — v7 은 «언제 만들어졌나» 를 노출한다** (2026-09-10, R 축). 앞 48비트가 곧 생성 시각(ms)이라
  **id 를 받은 사람은 그 행이 생긴 시각을 밀리초까지 되돌릴 수 있다.** 회원 PK 라면 그것이 **가입 시각**이다.
  ⚠ **실측**: 상품 문의 목록을 **토큰 없이** 부르는 것만으로 회원 세 명의 가입 시각을 초 단위로 복원했다
  (`handoffs/2026-09-10-handoff.md` §16-2).
  🔴 **이건 「보안 결함」이 아니라 «이 PK 선택의 성질»이다** — 위 「왜 v7」의 인덱스 이점과 **같은 한 가지 사실**의
  양면이다. 시간순으로 쌓이니까 빠르고, 시간순이니까 시각이 읽힌다. **한쪽만 가질 수 없다.**
  → **규칙**: 응답에 원시 PK 를 실을 때 **«이 값이 그 리소스 자신을 가리키는가»** 를 묻는다.
  «누가 썼나»(사람의 id)는 리소스의 신원이 아니다 — 그런 칸은 **판정 결과**(`mine` 같은 불리언)로 바꾼다.
  ⚠ **손으로 지키지 않는다** — `PublicResponseExposureTest` 가 공개 응답의 UUID 칸을 세고,
  `ALLOWED` 에 이유와 함께 없으면 빨개진다(BACKLOG §R-4).
...
>
> **2026-07-20 마감**: ①구매 인증 범위 수정 — 주문 상태 확장(7/16) 때 누락돼 결제·배송 완료 고객이
> 리뷰를 못 쓰던 버그. 새 상태가 **자동으로 포함되지 않도록** `<> CANCELLED` 가 아닌 opt-in 으로 둔다.
> ⚠ **여기에 상태를 열거하지 않는다** — 원본은 `OrderStatus.isPurchaseProven()` 하나다(아래 참조).
> ②**포토 리뷰** — `review.image_group_id`로
> ImageGroup 재사용 구조의 두 번째 사용처(FK 없는 느슨한 UUID, `ImageService` 공개 API로만 접근).
> ③상품 목록 평균별점은 **비정규화 + 이벤트 동기화**(§6.0 이벤트 항목 참조).
>
> ✅ **①의 「명시적 열거」는 2026-08-10 에 다시 어긋난 것이 확인돼 2026-08-11 에 고쳤다.**
> `DELIVERED`(V13, **07-23**)가 열거에 안 들어와 배송완료 고객이 리뷰를 못 쓰고 있었다(운영 3조합).
> **두 번째였다** — 판단(자동 포함 금지)은 옳았지만 **opt-in 은 주석으로 지켜지지 않았고**,
> 열거를 지키던 통합 테스트도 **같은 모양이라 같이 낡았다**.
> → 판정을 `OrderStatus.isPurchaseProven()` 으로 옮겼다. **`default` 없는 switch** 라 상태를 추가하면
> 운영·테스트 양쪽에서 **컴파일이 깨진다** — opt-in 을 규율이 아니라 컴파일러가 지킨다.
> ⚠ **이 문서에 그 목록을 옮겨 적지 않는다.** 여기 열거를 복사해 둔 것이 세 번 낡은 원인이다.
> 경위: `handoffs/2026-08-10-handoff.md` §16-2 · `handoffs/2026-08-11-handoff.md` §3-1.
>
...
>
> **✅ 2026-08-06 (G-3 1단계) 관리자 문의 목록** — `GET /api/admin/inquiries` (`AdminInquiryResponse`).
> 그전까지 관리자가 문의를 보는 길은 **상품 상세의 문의 탭뿐**이라, 「어느 상품이든 답을 기다리는
> 문의」를 볼 자리가 없었고 **상품 없는 문의는 답할 경로가 아예 없었다** — G-3(일반 문의)이 여기에
> 막혀 있었다. 세 가지가 이 자리의 판단이다:
> - ⚠ **새로 만든 것은 「찾는 길」뿐이다.** 답변 API 는 `POST /api/inquiries/{id}/answer` 로 이미
>   **상품 경로 밖**에 있어 그대로 재사용한다(첫 답변 알림 규칙도 그대로 따라온다).
> - 🔴 **관리자 목록은 마스킹하지 않는다.** `InquiryResponse` 의 비밀글 마스킹은 «볼 권한이 없는
>   사람» 을 위한 것이고 관리자는 거기서도 열람 갈래다 — 답을 쓰라면서 질문을 가릴 수 없다.
>   그래서 **DTO 를 따로 둔다**(고객 DTO 재사용 시 목록은 멀쩡히 뜨는데 본문만 자물쇠가 된다).
> - ⚠ **`status` 기본값을 서버가 정하지 않는다.** 화면이 「답변대기」로 열되, API 가 그걸 박으면
>   「전체」를 볼 방법이 사라진다(관리자 리뷰의 `hidden` 세 상태 규약과 같다).
>
> ⚠ **2단계(`product_id` nullable)를 위한 사전 확인**: 상품별 목록은 `productId.eq(...)` 라
> **NULL 행이 구조적으로 안 잡힌다** — 일반 문의가 생겨도 상품 문의 목록에 섞이지 않는다.
> 신경 쓸 곳은 **새로 만든 관리자 조회 하나뿐**이고, 거기선 상품명이 없어도 **줄을 남긴다**.
> → **2026-08-07 실측으로 확인됨**(아래).

> **✅ 2026-08-07 (G-3 2·3단계) 일반 고객센터 문의 · 내 문의** — V42. `inquiry` 한 테이블에
> **두 종류**가 산다: 상품 문의(`product_id` 있음 · `type=PRODUCT`)와 일반 문의(`product_id` **null**).
> 별도 `support_ticket` 도메인을 안 만든 이유는 **답변 API·답변 알림(B-15)·관리자 목록이 전부
> 재사용**되기 때문이다 — 나누면 관리자가 **두 목록을 번갈아 봐야** 하고, 그건 1단계가 방금 없앤 문제다.
>
> - 🔴 **3단계를 2단계와 나눌 수 없었다.** 답변 알림 링크가 `/products/{productId}#inquiries`
>   하나뿐이라 일반 문의는 **착지할 URL 이 없다** — 2단계만 배포하면 `/products/null#inquiries` 가
>   나간다. ⚠ 그리고 **서버 로그에 아무것도 안 남는다**(틀린 게 코드 경로가 아니라 **문자열**이다).
>   그래서 「내 문의」(`/support`)는 편의가 아니라 **일반 문의의 주소**이고, 링크는 `/support#inquiry-{id}` 다.
>   ⚠ 갈래는 `type` 이 아니라 **`productId` 로 가른다** — 필요한 판단이 «무슨 유형인가» 가 아니라
>   **«상품 URL 을 만들 재료가 있는가»** 라서다.
> - 🔴 **`product_id` 와 `inquiry_type` 은 같은 사실의 두 면이다.** 컬럼이 둘이라 한쪽만 틀린 행이
>   생길 수 있고, 그런 행은 **앱을 멈추지 않는다** — 관리자 목록의 상품명이 영원히 비거나(PRODUCT인데
>   상품 없음), 일반 문의가 **상품 문의 목록에 섞여 뜬다**(일반 유형인데 상품 있음).
>   → **엔티티 생성자 + DB `ck_inquiry_product_pair` 양쪽**에서 막는다. 앱 방어는 «경로가 하나 더
>   생기면 새는» 종류라 DB 가 마지막에 잡는다(G-2 의 «환영 쿠폰 한 장» 을 유니크 인덱스로 못 박은 것과 같다).
>   ⚠ 그 제약은 **앱에서 한 번도 안 밟히므로** `esptest` 에서 직접 INSERT 로 확인한다(WA §2-4-2).
> - ⚠ **`inquiry_type` 값은 처음부터 6종**(PRODUCT·DELIVERY·REFUND·PAYMENT·ACCOUNT·ETC).
>   PAYMENT·ACCOUNT 는 화면에 안 띄우고 **값만 열어 뒀다** — Oracle 은 나중에 enum 을 늘리면
>   CHECK 를 못 고쳐 수동 ALTER 다. 넣어 두는 비용은 0 이고 나중 비용은 수동 ALTER 라 비대칭이다.
> - ⚠ **PRODUCT 는 사용자가 고르는 값이 아니라 경로가 정한다.** 그래서 일반 작성 요청 DTO 를 상품용과
>   **따로 뒀다**(`GeneralInquiryCreateRequest`) — 공용으로 쓰면 상품 경로로 보낸 `type` 이 조용히
>   덮여 **보낸 값이 사라진 채 200** 이 나간다. 무시되는 입력 필드는 있으나 마나가 아니라 **거짓말**이다.
> - ⚠ **`product_id` 는 `updatable = false` 를 유지한다** — nullable 로 여는 것과 «한 번 정해지면
>   못 바꾼다» 는 다른 이야기다. 붙일 수 있게 하면 이미 나간 답변·알림 링크가 어긋난다.
> - **내 문의는 두 종류를 가르지 않는다** — 사용자는 자기 문의가 상품에 달렸는지 고객센터에 냈는지
...
🔴 **③ 영구 삭제 때 «상품을 가리키던 줄» 은 세 갈래로 갈린다** (2026-09-10 M-4 · 2026-09-11 O-7):

| 갈래 | 테이블 | 어떻게 |
|---|---|---|
| **상품에 종속된 기록** | 옵션 · 재고 이력 · 기간 할인 | FK `ON DELETE CASCADE` |
| **상품을 가리키기만 하는 것** | 찜 · 재입고 구독 · 알림 링크 | `ProductPurgedEvent` 를 **각 도메인이 받아** 자기 것만 지운다(도메인 간 FK 가 없다) |
| **사람이 쓴 것 · 원장** | 리뷰 · 문의 · 주문 품목 | **남긴다** — 상품이 없어도 읽을 값이 있다(이름·단가를 스스로 들고 있다) |

→ 가르는 기준은 **«상품이 없어지면 그 줄에 남는 것이 있나»** 다. ⚠ 새 테이블에 `product_id` 를 넣으면
`ProductPurgeReferenceIntegrationTest` 가 **셋 중 하나를 고르게** 한다(DB 의 칸 목록에서 꺼낸다 — WA §2-12).

### 쿠폰의 기간은 둘이다 — 발급 창 · 사용 기간 (2026-08-13, G-8 · V49)

🔴 **「언제 받을 수 있나」와 「언제까지 쓸 수 있나」가 그전까지 같은 값이었다.** `coupon` 의 기간
컬럼은 `valid_from`·`valid_until` 뿐이었고 그 값을 `redeem` 이 **사용 가능 판정**에 썼다.
→ 이벤트 쿠폰(«그 날 접속해서 받는다»)을 그 값으로 만들면 **그 날 자정에 만료돼 받자마자 못 쓴다.**

| 무엇 | 컬럼 | 값 예 |
|---|---|---|
| 이벤트 날(발급 시작 = 사용 시작) | `valid_from` | 8/15 00:00 |
| **발급 마감** | **`issue_until`**(신설, nullable) | 8/15 23:59 |
| 사용 마감 | `valid_until` | 9/15 23:59 |

- ⚠ **`issue_until IS NULL` = 이벤트가 아닌 상시 쿠폰**이다. 별도 플래그를 두지 않았다 —
  `welcome`(V36)은 날짜만으로 «어느 것이 가입 쿠폰인지» 를 알 수 없어 플래그가 필요했지만,
  이벤트 쿠폰은 **발급 마감일이 곧 정의**다. 플래그를 두면 «켜졌는데 날짜가 없다» 는 모순 상태가
  생기고 그걸 앱이 다시 막아야 한다.
- ⚠ **`issue_from` 은 두지 않았다** — 발급 시작과 사용 시작이 항상 같으면 `valid_from` 의 복사본이다.
  *"미리 받아 두고 다음 달부터"* 가 필요해지면 그때 더한다(**null = `valid_from`** 으로 읽으면 백필이 없다).

🔴 **겹침 금지의 대상은 발급 창이지 사용 기간이 아니다.** 8/15 쿠폰과 8/20 쿠폰의 사용 기간은
9월 중순까지 나란히 살아 있어야 정상이라, 여기를 헷갈리면 **두 번째 이벤트부터 등록이 통째로 막힌다.**

🔴 **그 금지는 앱 검사뿐이다 — Oracle 유니크 인덱스로는 기간 겹침을 못 막는다.**
그래서 두 가지가 따라온다:
- «겹치는 둘을 등록» 을 **테스트로 못 박는다**(DB 가 안 받쳐 주므로 회귀를 잡을 곳이 거기뿐이다).
- **두 관리자가 같은 순간 등록하면 둘 다 통과할 수 있다.** 그때 «열린 이벤트» 조회가 `Optional` 이면
  `NonUniqueResultException` 이 나고 그건 **홈에 들어오는 모두에게 500** 이다 — 배너 하나 때문에
  첫 화면이 죽는 건 과하다. → 여럿이면 **가장 먼저 시작한 것**을 쓰고 **경고 로그**를 남긴다
  (조용히 고르지 않는다 — 사람이 고쳐야 하는 상태다).

**중복 발급은 DB 가 막는다** — `ux_member_coupon_once(member_id, coupon_id)`(V49).
앱 가드는 흔한 경우에 뜻이 있는 4xx 를 주기 위한 것이고, **동시에 누른 두 요청은 앱이 못 막는다**
(둘 다 «없다» 를 읽는다). ⚠ 이 인덱스가 **기존 동작을 바꿨다**: `CouponService.issue`(관리자 발급)는
*"같은 쿠폰을 여러 장 주는 것도 허용"* 이었는데, **회원당 한 장**이 전역 규칙이 됐다(2026-08-13 결정).

**만료(사용 마감이 지남)는 «쓰는 자리» 셋에서 막는다** (2026-09-17, `handoffs/2026-09-17-handoff.md` §3·§4) —
그전엔 셋 다 기간을 안 봐 **끝난 쿠폰이 나갔다**. 경계는 한 곳 `Coupon.isExpiredAt`(시작 전은 만료가 아니다):

| 자리 | 만료면 |
|---|---|
| 가입 쿠폰 **지정** | 거절(`COUPON-400X`) · 이벤트 쿠폰도 거절(`400G` — 가입 발급과 「받기」가 한 쿠폰에서 섞인다) |
| 가입 쿠폰 **시간 경과** | 지정은 두고 **«없음» 으로 취급** — 공개 안내 null · 자동 발급 건너뜀 + 경고 (배치로 지정을 풀지 않는다: 사람이 안 누른 조작은 감사에 못 남는다) |
| 관리자 **수동 발급** | 거절(`400F`). ⚠ 이벤트 **발급 창**은 안 본다 — «그 날 접속한 사람만» 은 「받기」의 약속이고 관리자 발급은 CS 통로다 |

**회수·정의 삭제는 행을 지운다** (2026-09-17, BACKLOG Q-7) — `revoked_at` 표시가 아니다. 흔적은 감사(`COUPON_REVOKE`·`COUPON_DELETE`)뿐이다.
- 🔴 **쓰인 발급분은 못 지운다** — 주문이 `member_coupon_id`(V46)로 가리켜 취소·반품 때 되돌릴 대상이다. 경합은 **조건부 DELETE**(`usedAt is null`)가 막는다.
- ⚠ **미사용인데 주문이 가리키는 행이 있다** — 전체 취소·반품이 되돌려 놓은 쿠폰이다. 그 주문은 더 되돌릴 것이 없어 지워도 된다.
- **정의 삭제는 발급분 0장일 때만**(FK) · 가입 쿠폰이면 지정부터 푼다. 쓰인 발급분이 있는 정의는 **영영 못 지운다**(주문은 쿠폰명을 스냅샷해 두어 정의가 없어도 멀쩡하지만, 발급분 행이 정의를 붙잡는다).
...
>
> ✅ **대상이 늘어난 경위 (2026-08-14, V50·V51)**: 회원 조작 → 주문 취소(V43) → 리뷰·문의 숨김(V44) →
> **상품 삭제·복구**(V50) → **관리자 주문 조작 넷**(V51: 발송·배송완료·반품 승인·거절).
> 🔴 **V50 에서 `target_id` 의 뜻이 넓어졌다** — 그전까지는 «대상 회원 id» 였고 주문은 **주문자**,
> 리뷰·문의는 **작성자**를 잡아 통과했는데 **상품에는 회원이 없다.** 지금은 «대상의 id — 무엇인지는
> `action` 이 말한다» 다(사용자와 확정). ⚠ **대가**: 감사 화면 검색이 `target_login` 부분일치뿐이라
> **상품 행은 「조작 종류」 필터로만 걸린다.** 회원 아닌 대상이 **하나 더 늘면** `target_type` 을 세울 때다 —
> 지금은 상품 하나뿐이라 안 만들었다(V51 은 대상을 주문자로 잡아 이 조건을 안 건드렸다).
> ⚠ **enum 값을 늘릴 때는 CHECK 제약을 함께 넓히고**(Oracle enum CHECK 트랩) **통합 테스트로 실제 INSERT 를
> 밀어 넣는다** — 목은 제약을 모르고, `@Transactional` 롤백만 하면 제약이 한 번도 실행되지 않는다.
>
> ✅ **정정 (2026-08-20, V53·V54)**: 위의 «대가» 는 **갚았고**, «하나 더 늘면» 이라는 조건도 **성립했다.**
> 상품 등록·수정 · 쿠폰 셋 · 할인 셋이 붙으면서 회원 아닌 대상이 **상품·쿠폰 둘**이 됐고,
> **`target_type`(`MEMBER`·`PRODUCT`·`COUPON`)을 세웠다.** 감사 화면에 「대상」 열과 「대상 종류」
> 필터가 생겨 **상품 행도 이제 좁힐 수 있다.**
> 🔴 **`target_type` 은 이벤트로 받지 않고 `AuditAction` 이 답한다**(`action.targetType()`) —
> 파라미터로 받으면 «action 은 `PRODUCT_UPDATE` 인데 targetType 은 `MEMBER`» 인 행을 만들 수 있고,
> **만들 수 있게 두면 언젠가 만들어진다.** 위 문장(*«무엇인지는 action 이 말한다»*)을 **사람이 읽는
> 주석에서 기계가 읽는 값으로 옮긴 것**이라, 기존 발행 지점 다섯은 한 줄도 안 고쳤다.
> ⚠ **값에 `DISCOUNT` 가 없다** — 할인 조작의 대상을 **상품**으로 잡았다. 할인 id 는 사람에게 의미가
> 없고, 상품으로 잡아야 상품 등록·수정·삭제와 **같은 `target_id` 로 묶여** «이 상품에 무슨 일이
> 있었나» 를 한 줄로 읽는다. V43 이 주문 조작의 대상을 «주문» 이 아니라 «주문자» 로 잡은 것과 같은 결이다.
> ⚠ **V54 는 V53 이 놓친 §2-2-1(`VARCHAR2` 는 예외 없이 `CHAR`)의 보정**이다 — 적용된 스크립트는
> 고칠 수 없어(체크섬) 다음 마이그레이션으로 덮었다.
> 근거: `handoffs/2026-08-20-handoff.md` §7.
>
> ✅ **4차 — `target_type` 이 처음으로 «값이 늘어» 확장됐다 (2026-08-21, V56)**:
> **카테고리 등록·삭제 · 공지 등록·수정·삭제 · 문의 답변**을 붙이며 `CATEGORY`·`NOTICE` 두 값을 세웠다.
> 🔴 **여기가 V53 구조의 첫 시험대였다.** 그전 넷(할인·주문·리뷰·문의)은 전부 **기존 값으로 접혔는데**
> 카테고리·공지는 `MEMBER`·`PRODUCT`·`COUPON` 어디에도 안 들어간다.
>
> **접는 길이 둘 다 있었고, 둘 다 안 갔다**:
> - **카테고리 → `PRODUCT`**(상품을 담는 그릇이니까) ❌ 접으면 `target_id` 는 카테고리 id 인데 종류는
>   «상품» 이라 **「대상 종류=상품」에 상품 아닌 행이 섞인다.** 그 필터의 존재 이유가 «회원 아닌 행을
>   갈라 보는 것» 인데 **접는 순간 그 쓸모를 스스로 무너뜨린다.**
> - **공지 → `MEMBER`**(공지에도 `author_id` 가 있다) ❌ 리뷰·문의가 «대상=작성자» 로 통한 이유는
>   **고객이 쓴 것을 관리자가 조치**해서다. 공지는 **관리자가 쓰고 관리자가 고친다** →
>   actor 와 target 이 거의 늘 같은 사람이라 **원장 한 줄이 아무것도 안 가른다.**
>
> → 🔴 **«대상은 id 를 물어볼 값어치가 있는 쪽»(V43·V53) 규칙 자체는 그대로 통했다.** 이번엔 그 답이
> 기존 셋 **밖**에 있었을 뿐이고, **틀을 고칠 일은 없었다 — 값만 늘었다. 그것이 시험의 결과다.**
> ⚠ **문의 답변은 `target_type` 을 안 건드린다** — 대상이 **질문자**라 `MEMBER` 이고, 덕분에 같은 회원의
> «숨김»(V44)과 «답변» 이 **한 `target_id` 로 묶인다.** detail 에 «첫 답변 / 답변 수정» 을 적는데,
> 🔴 되돌릴 수 없는 것이 답변이 아니라 **알림**이라(B-15 는 첫 답변에만 보낸다) **그 한 마디가
> «알림이 나갔나» 의 답**이다.
> ⚠ **공지가 이제야 대상이 된 것은 E-4 덕이다**(2026-08-20) — 그전에는 고객도 공지를 써서
> **«관리자 조작» 이 아니었다.** 즉 이 값은 기능 하나가 닫히면서 **비로소 뜻을 갖게 된** 값이다.
> ⚠ **이미지 파생 생성은 안 붙였다** — 운영 배치성이고 되돌릴 수 있으며 «누가» 를 물을 일이 없다.
> **전수 조사에서 뽑은 넷을 다 붙이는 것이 목표가 아니었다.**
> ⚠ **백필 안 함** — 없는 것이 «누가·언제» 라 지어내는 것이 된다(V44·V50·V51 과 같은 판단).
> V53 의 `target_type` 백필은 **action 에서 계산되는** 값이라 경우가 달랐다.
> 근거: `handoffs/2026-08-21-handoff.md` §3.
>
...
> | | 대상 |
> |---|---|
> | **지운다** | 배송지 · 찜 · 적립금(계정+이력) · 보유 쿠폰 · 알림 · **알림설정** · 재입고 구독 · **문의** · **장바구니**(2026-08-11) |
> | **남긴다** | **주문**(매출 집계 근거) · **리뷰**(별점 집계 + 다른 고객의 판단 근거) · **공지**(관리자 콘텐츠) |
>
> ⚠ **«공지=관리자 콘텐츠» 는 2026-08-20 까지 문서에만 있었다**(BACKLOG E-4). 실제 배선은 네 층이
> 전부 «아무나 쓸 수 있다» 였다 — `SecurityConfig` 가 `authenticated`, 서비스 `create` 에 가드 없음,
> 라우터가 `requiresAuth`, 「새 공지」 버튼이 `v-if="isLoggedIn"`. **화면조차 안 막았으므로
> 「화면이 유일한 방어」 사례도 아니었다.** 🔴 **08-20 에 네 층을 다 좁혔고**(`hasRole("ADMIN")`),
> 서비스의 `isAdmin() || isOwnedBy()` 는 **도달 불가가 되어 지웠다**(같은 규칙을 두 곳이 지키면
> 한쪽은 죽은 코드다). ⚠ 이미 일반 회원이 쓴 공지 1건은 **그대로 둔다**(검증 데이터) —
> 그 작성자는 이제 자기 글도 못 고친다. 근거: `handoffs/2026-08-20-handoff.md` §11.
>
> 🔴 **장바구니는 2026-08-11 에야 들어왔다**(08-10 §16-4 6번). 나머지 여덟은 전부 DB 테이블인데
> 장바구니만 **Redis**(`cart:{memberId}`)라, F-1 을 만들 때 「지울 **테이블**」을 세면서
> 「지울 **저장소**」를 안 셌다 — **목록의 단위가 틀리면 항목이 빠진다.**
> 실측(2026-08-11): 운영 Redis 의 `cart:*` **96개가 전부 주인 없는 것**이었다(회원 6명, 매칭 0개).
> ⚠ TTL 30일이 결국 지우므로 영구 누수는 아니었지만, F-1 의 성격은 «언젠가 사라진다» 가 아니라
> **«탈퇴하면 지운다»** 다. ⚠ Redis 삭제는 **롤백되지 않는다** — 반대(개인정보가 남는 쪽)보다 낫다는
> 판단이고, `RefreshTokenStore`·`TokenRevocationStore` 정리가 이미 같은 자리다.
> → 이 표를 **손으로 지키지 않는다**: `MemberPurgeIntegrationTest` 가 정리 후 **회원 id 가 든 Redis 키를
> 훑어** 남은 것이 있으면 실패한다(예외는 `auth:revoked-before:` 하나뿐 — E-2 가 일부러 써 넣는 값).
>
...
>
> ~~⚠ **감사 로그는 남기지 않는다**~~ — 🔴 **2026-09-11 에 뒤집었다(V64 · BACKLOG O-6).** 아래가 그때의 근거고,
> **둘 다 무너졌다**:
> `admin_audit_log` 는 `target_id`/`target_login` 이 **NOT NULL** 인
...
> → ①더미 대신 **`target_type=MARKETING` 일 때만 `target_id` 를 비운다**(`ck_admin_audit_target_id` 가 짝을 쥔다 —
> «거짓을 쓰지 않는다» 는 원칙은 그대로다) ②🔴 **F-2(2026-09-10)가 보관 기간이 지난 알림을 지운다** —
> «알림 행이 기록» 이던 전제가 30~180일 뒤 사라진다. 이제 `MARKETING_SEND`(제목·동의·발송 수)가 남는다.
> ⚠ **한 결정의 근거가 다른 기능(F-2)에서 무너졌는데 그 결정은 여기에만 적혀 있었다** — 09-11 에 «빠진 것» 으로
> 잘못 읽힌 경위는 `handoffs/2026-09-11-handoff.md` §6.
>
> ⚠ **정정 (2026-08-14)**: 위의 *"`target_id`/`target_login` 이 NOT NULL"* 은 **더 이상 사실이 아니다** —
> `target_login` 은 **V45(2026-08-10)로 nullable** 이 됐다(탈퇴 회원의 콘텐츠를 조치할 수 있어야 한다).
> `target_id` 는 여전히 NOT NULL 이고 **결론은 그대로다**: N명 브로드캐스트는 대상이 «한 명» 이 아니라
> 안 맞는다. ⚠ 다만 이유가 «NOT NULL 두 개» 에서 **«대상이 하나로 특정되지 않는다»** 로 좁혀졌다.
>
> 🔴 **재검토 (2026-08-20, 사용자 요청)**: **결론은 또 그대로인데 이번엔 근거가 바뀐다.**
> «모델이 안 맞는다» 는 이제 **약한 이유다** — 같은 날 «값을 복제하지 말고 **«일이 있었다 + 어디를
> 보라»** 를 적는다» 는 해법이 나왔고(재고를 감사 detail 에 안 적은 자리), 그대로 쓰면 `detail` 에
> «동의 N명 중 M명 발송 · 제목» 을 적고 **개별 수신자는 `notification` 이 갖는다.** 남는 문제는
> `target_id`(NOT NULL) 하나뿐이고 ①nullable 완화 + `BROADCAST` 값 ②발송마다 «발송 id» 발급으로 풀린다.
> **즉 «못 한다» 가 아니라 «안 한다» 다.**
> ⚠ **그런데 실측이 결론을 유지시킨다**(2026-08-20): `notification` 의 `MARKETING` 행 **0** ·
> 발송 **0회** · 마케팅 동의 회원 **0명**(전체 6명). **지금 붙이면 한 줄도 안 쌓인다.**
> 🔴 **재검토 조건**: 동의자가 생기고 실제 발송이 한 번이라도 일어나면 그때 붙인다.
> ⚠ 그 김에 나온 것 둘(BACKLOG): **누가 보냈는지가 로그에도 없다**(`log.info` 에 actor 없음 ·
> `Notification` 에 발송자 컬럼 없음 · 저널은 휘발성) · **이 기능은 한 번도 관통된 적이 없다**(동의자 0).
> 근거: `handoffs/2026-08-20-handoff.md` §12.
...
> **가격이 세 층이다 — 정가 · 세일 전 판매가 · 실판매가 (2026-08-19 V52 · 2026-08-20 V55)**
>
> 기간 할인(타임세일, G-5)이 들어오면서 «가격» 이 **하나가 아니게 됐다.** 셋을 섞으면 돈에서 어긋난다:
>
> | 값 | 뜻 | 누가 정하나 | 어디에 |
> |---|---|---|---|
> | **정가** `list_price` | «원래 이 값어치»(권장소비자가) | 관리자가 **손으로**. **비어 있을 수 있다** | `product` · `order_item` |
> | **세일 전 판매가** `regular_price` | «이 세일이 없었으면 받았을 금액» = 기본가 + 옵션 가격차 | **서버가 계산** | `product`(응답) · `order_item`(V55) |
> | **실판매가** `price` | «지금 청구되는 금액» | 서버가 계산(세일 적용) | 전부 |
>
> 🔴 **`price` 에 세일을 실은 것이 V52 의 핵심 결정이다.** 반대로(«`price` 는 원가, `salePrice` 를 새로»)
> 갔으면 프론트에서 `product.price` 를 읽는 **7곳**을 전부 `salePrice ?? price` 로 고쳐야 하고
> **한 곳만 빠져도 세일이 조용히 안 먹는다.** 대신 «세일 전» 이 필요한 자리(관리자 수정 폼)가
> `regularPrice` 를 읽는다 — 거기서 `price` 를 실으면 **할인이 두 번 먹는다.**
> ⚠ **곱 순서를 못 박았다**: `(기본가 + 가격차) × (100 − 할인율) / 100` — 가격차를 **더한 뒤** 할인율이다.
> 반대면 «L +2,000» 옵션만 할인이 덜 먹어 **옵션마다 체감 할인율이 달라진다.**
> 🔴 실측(2026-08-20)으로 갈렸다: `L` 이 **9,600** — 뒤집으면 10,000 이라 값이 다르다.
>
> 🔴 **주문은 셋 중 둘을 스냅샷한다**(`price` · `list_price` · `regular_price`).
> ⚠ **`regular_price`(V55)가 없던 동안 «세일로 샀다» 가 주문에 안 남았다** — 정가 칸이 빈 상품을
> 세일가로 사면 **실판매가만 남고 끝**이었다(실측 `20260820-4733`: `price 9600` · `list_price NULL`).
> 상품이 지워지면 `product_discount` 도 CASCADE 로 사라지므로 **되짚을 근거가 어디에도 없어진다.**
> ⚠ **백필하지 않았다** — 08-19 이전 주문은 `= price` 가 맞지만 그 뒤 둘은 **실제 세일가**라
> 거짓을 적게 된다. `NULL` 은 «그때는 이 값을 안 남겼다» 는 사실이다.
> 근거: `handoffs/2026-08-19-handoff.md` · `handoffs/2026-08-20-handoff.md` §13.
...
>
> 🔴 **정정 (2026-08-14)**: 위의 전제는 **틀렸었다.** 리뷰에는 작성자가 있어 `targetId` 를 채울 수 있고,
> **V44(2026-08-10)가 `REVIEW_HIDE`/`INQUIRY_HIDE` 로 실제로 붙였다.** 그때 갈리는 기준은
> *"도메인이 아니라 «대상에 회원이 있느냐»"* 로 정리됐다.
> 🔴 그리고 **그 기준마저 2026-08-14 에 넓어졌다**(V50, `PRODUCT_DELETE`/`PRODUCT_RESTORE`) —
> 상품에는 회원이 없어서 `target_id` 의 뜻을 **«대상의 id, 무엇인지는 `action` 이 말한다»** 로 넓혔다.
> ⚠ 회원 아닌 대상이 **더 늘면** `target_type` 열을 세울 때다(지금은 상품 하나뿐이라 안 만들었다).
