...
🔴 **DB 만 뜨면 이미지를 잃는다** — DB 에는 이미지 **경로**만 있다.
🔴 **VM 안의 백업은 백업이 아니다** — 원칙은 그대로다. ✅ **그럼에도 호스트로 가져가지 않기로 했다**
(2026-09-18 사용자 결정 — *"어짜피 이 vm에서 할 거니까"*). ⚠ **다시 권하지 않는다**(`backup-db.sh` 가 매번 찍던 `scp` 안내를 2026-09-21 에 뺐다).
🔴 **대가를 아는 채로 고른 것이다**: **VM 자체를 잃으면 백업도 함께 잃는다.**
