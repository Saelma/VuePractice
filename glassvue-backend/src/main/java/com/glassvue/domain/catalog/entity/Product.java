...
public class Product extends BaseTimeEntity {
     ...
     *         멱등 호출이 감사에 두 줄을 남기는 것을 막는 자리다
     *         (멱등을 «에러로 만들지 않는다» 는 판단은 그대로 두고, <b>기록만</b> 가른다).
     *
     *         <p>⚠ <b>«복구 화면에서 두 번 누르면» 이 아니다</b>(2026-08-14 브라우저 검증에서 확인).
     *         복구하면 그 줄이 <b>삭제 대기 목록에서 곧바로 빠져</b>({@code onRestore} 가 목록을 다시
     *         받는다) 한 탭에서는 <b>두 번째 버튼이 존재하지 않는다.</b> 실제로 닿는 경로는
     *         <b>낡은 목록</b>이다 — 두 관리자(또는 두 탭)가 같은 줄을 열어 두고 차례로 복구하면
     *         뒤엣것이 <b>이미 복구된 상품</b>을 친다. 관리자가 셋이라 있을 수 있는 일이다.
     */
