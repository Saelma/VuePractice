 ...
 */
public record OrderPlacedEvent(UUID orderId, UUID memberId, long totalPrice, int itemCount) implements DomainEvent {

    public static OrderPlacedEvent from(Order order) {
        return new OrderPlacedEvent(order.getId(), order.getMemberId(), order.getTotalPrice(), order.getItems().size());
    }
}
