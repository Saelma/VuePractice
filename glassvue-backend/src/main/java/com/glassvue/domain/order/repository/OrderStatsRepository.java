 ...
 *       <b>여기에 목록을 옮겨 적지 않는다.</b> ⚠ 옮겨 적었다가 새 상태가 생겼을 때
 *       <b>사본이 다 안 자란</b> 적이 있다 — <b>복사본이 있으면 그 복사본이 낡는다.</b>
 ...
 *       전량이 빠져야 {@code CANCELLED}·{@code RETURNED} 로 떨어져 상태 필터에서 빠진다.</li>
...
public interface OrderStatsRepository extends Repository<Order, UUID> {
     ...
     *
     * ⚠ **상태 목록을 `OrderStatus.isRevenue()` 한 곳에 둔 것과 같은 이유**다. 🔴 **같은 사고가
     *   두 번 났다** — 한 번은 상태 목록에서, 한 번은 이 금액 식에서(사본 넷이 함께 낡아 운영
     *   매출이 부풀려진 채 돌았다). ⚠ **테스트 사본도 같이 틀려서 아무것도 안 빨개졌다** —
     *   WA §1-2-1: *「손으로 열거하는 코드를 손으로 열거하는 테스트로 지키면 둘이 같이 어긋난다」*.
     *   경위는 `handoffs/2026-08-25` §I-1.
     *
