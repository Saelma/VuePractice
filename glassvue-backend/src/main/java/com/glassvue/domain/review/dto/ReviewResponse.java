 ...
 *
 * <p>{@code author} 는 본인·관리자가 아니면 가린다({@link AuthorNames}, R-7).
 */
public record ReviewResponse(
    ...
    public static ReviewResponse from(Review r, AuthUser viewer, List<ImageResponse> images) {
        boolean mine = viewer != null && r.getAuthorId() != null && r.getAuthorId().equals(viewer.id());
        return new ReviewResponse(
                r.getId(),
                r.getProductId(),
                mine,
                AuthorNames.forViewer(r.getAuthor(), mine, viewer),
                r.getRating(),
                r.getContent(),
                images,
                r.getCreatedAt(),
