...
> ```bash
> ssh -L 8025:127.0.0.1:8025 -L 3000:127.0.0.1:3000 -p 2222 ecstel@127.0.0.1
> ```
> 이후 개발 PC 브라우저에서 `http://localhost:3000`(화면)·`http://localhost:8025`(메일함).
>
> ⚠ **서버 IP 로 접속하지 않는다**(2026-07-30) — VM 이 NAT 라 호스트에서 **포트포워딩**으로 들어간다
> (`127.0.0.1:2222` → VM `:22`). 운영 화면도 같은 이유로 `https://localhost/`(443 포워딩)다.
> IP 는 두 번 바뀌었고(`.36` → `.14` → NAT), 그때마다 IP 를 적어 둔 문서가 전부 낡았다.
